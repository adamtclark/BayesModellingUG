...
Today will just be the "Vorbesprechung" (pre-Semester meeting). For anyone who would has questions about the course or about the semester, we can meet at this link <https://unimeet.uni-graz.at/b/cla-gru-xu4-48a>.

Next week, the plan is to meet as usual at 15.30 in seminar room 32.11 (to floor of Holteigasse 6). No preparation is needed for this meeting, but if possible, please bring a laptop with R already installed.
...
Obviously, we get more positive test results in the high scenario. BUT, we can also use this result to calcualte the proability that we wanted at the beginning, i.e. P(A|B), the probability that you are sick given that you test positive. To do this, we run:
...
Notice that in the low scenario, the probability of being sick if you test positive is actually quite low - just around 45%. This is because the false positive rate is roughtly the same as the overall incidence of the disease in the population (so, per person tested, a false positive is just about as likely as it is to be sick in the first place). On the other hand, in the "high" scenario, the false positive rate is just about 0% - that is, almost everyone who tests positive is actually sick.

Finally, we can work this out exatcly with the Bayes formula itself. To do so, we actually need to expand P(B) a bit, since "probability of the test" doesn't really make much sense as a concept. Instead, we break this into two different probabilities:
